...
class CompletePipeline:
    ...
    def _plot_poem_line_chart(self, results_df, metric_col):
        """Create line chart for poem grid search"""
    ...
    def _plot_comparison_chart(self, verse_results, poem_results, verse_metric, poem_metric):
        ...
        labels = [f"p{int(row['num_perm'])}_s{int(row['shingle_size'])}_t{row['threshold']:.2f}" 
    ...
    def _generate_report(self):
        ...
        report = [
            ...
            f"  Has ground truth: verse={self.has_verse_gt}, poem={self.has_poem_gt}",
            ...
            f"  Avg verses per cluster: {len(self.df)/n_verse_clusters:.1f}",
            f"  Avg poems per cluster: {len(self.poem_clusters)/n_poem_clusters:.1f}",
            "\nTIMING (wall-clock):",
            f"  Data loading: {self.timing['load']:.1f}s",
            f"  Verse clustering (incl. grid search): {self.timing['verse_clustering']:.1f}s ({self.timing['verse_clustering']/60:.1f} min)",
            f"  Poem clustering (incl. grid search): {self.timing['poem_clustering']:.1f}s ({self.timing['poem_clustering']/60:.1f} min)",
            f"  TOTAL: {self.timing['total']:.1f}s ({self.timing['total']/60:.1f} min)",
            "\nTHROUGHPUT:",
            f"  Verses/second: {len(self.df)/self.timing['total']:.0f}",
            f"  Poems/second: {len(self.poem_clusters)/self.timing['total']:.0f}",
            "\nCOMPUTATIONAL RESOURCES:",
            f"  System: {cpu_count} physical cores, {cpu_count_logical} logical cores, {total_mem:.1f} GB RAM",
            f"  Parallel jobs used: {self.n_jobs if self.n_jobs != -1 else cpu_count_logical}",
            ...
            f"  Data loading: {self._format_stage_resources(0, 1)}",
            f"  Verse clustering: {self._format_stage_resources(1, len(self.resources['cpu_percent'])//2)}",
            f"  Poem clustering: {self._format_stage_resources(len(self.resources['cpu_percent'])//2, -1)}",
            "\nOUTPUT FILES:",
            ...
            f"  {self.output_dir}/verse_grid_heatmap.png",
            f"  {self.output_dir}/verse_grid_analysis.png",
            f"  {self.output_dir}/poem_grid_analysis.png",
            f"  {self.output_dir}/comparison_chart.png",
            "="*70
        ]
